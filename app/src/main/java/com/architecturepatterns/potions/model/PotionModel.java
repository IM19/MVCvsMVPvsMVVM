...
public final class PotionModel implements IPotionModel {
    ...
    @NonNull
    @Override
    public LiveData<List<Potion>> getPotionsList() {
        MutableLiveData<List<Potion>> potionList = new MutableLiveData<>();
        potionList.setValue(mPotionsList);
        return potionList;
    }

    @NonNull
    public LiveData<Potion> getPotionDetails(int potionID) {
        MutableLiveData<Potion> potion = new MutableLiveData<>();
        if (mPotionsList.size() != 0) {
            potion.setValue(mPotionsList.get(potionID));
        }
        return potion;
    }
